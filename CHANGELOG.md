...
### 工时记录 / 超期
- **完整模式也能判超期**：原超期判定 `taskPeriod` 只读模板里的 `_jhKsDate/_jhJsDate`（仅懒人模式有），完整模式拿不到周期 → 永不标超期/不警告。改为与「本次报工任务」横幅一致：按 `gstbRwId` 从已加载的 `getMyTask` 列表匹配出周期，**两种模式都能判超期**（红标 / 提交前警告）。
- **历史记录不再误标超期**：超期红标原在两 tab 共用列里渲染、按"当前配置任务"的周期比对，导致**换到周期更窄的任务后，以前已提交的历史记录被假标超期**（记录不绑任务、无法知道当初属于哪个任务）。改为**超期红标只在"待处理" tab 显示**（它们才会按当前任务提交）；历史记录已提交、可能属于别的任务，不再显示超期。
- 浏览器端到端验证：完整模式下待处理超期项标红、提交前正确列出并软拦截；历史 tab 同一超期日期不再标红。
